...
function pressKey(event){
    let letter = event.key.toUpperCase() //litera care a fost apasata
    //gasim primul element div care are clasa literei apasate
    let div = document.querySelector("." + letter)
    // facem div-ul sa dispara 
    div.remove()

    // se gasesc toate elementele div care au clasa literei selectate
    // let divs = document.querySelectorAll("." + letter)
    // dispatchEvent.forEach(div => div.remove())
}
